...
function SoundEffectsView() {
	...
	useEffect(() => {
		// Desktop build: top-sounds discovery is disabled until the local
		// extraction tool is integrated. Saved sounds still load above.
		if (hasLoaded) return;
		setTopSoundEffects({ sounds: [] });
		setHasLoaded({ loaded: true });
		setLoading({ loading: false });
		setError({ error: null });
	}, [hasLoaded, setTopSoundEffects, setHasLoaded, setLoading, setError]);
